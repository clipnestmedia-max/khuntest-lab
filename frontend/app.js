...
const defaultStaff=[{id:'ST1',name:'Ravi Kumar',phone:'9876500001',area:'Darbhanga'},{id:'ST2',name:'Amit Jha',phone:'9876500002',area:'Laheriasarai'}];
...
function init(){if(!localStorage.getItem(K.tests))set(K.tests,defaultTests);if(!localStorage.getItem(K.packages))set(K.packages,defaultPackages);if(!localStorage.getItem(K.staff))set(K.staff,defaultStaff);if(!localStorage.getItem(K.bookings))set(K.bookings,[{id:'KTB1001',patientName:'Demo Patient',age:'28',gender:'Male',phone:'9876543210',email:'demo@patient.com',test:'Full Body Checkup',price:1999,bookingType:'Home Collection',date:'2026-06-05',time:'09:00',address:'Darbhanga, Bihar',status:'Confirmed',payment:'Paid',staff:'Ravi Kumar',createdAt:new Date().toISOString(),reportName:'',reportData:''}]);if(!localStorage.getItem(K.payments))set(K.payments,[]);if(!localStorage.getItem(K.notifications))set(K.notifications,[])}init();
...
function bookTest(e){e.preventDefault();const f=e.target;const opt=f.test.options[f.test.selectedIndex];const price=Number(opt?.dataset?.price||0);const booking={id:uid('KTB'),patientName:f.patientName.value.trim(),age:f.age.value,gender:f.gender.value,phone:f.phone.value.trim(),email:f.email.value.trim(),test:f.test.value,price,bookingType:f.bookingType.value,date:f.date.value,time:f.time.value,address:f.address.value.trim(),status:'Pending',payment:f.payment.value,staff:'Not Assigned',createdAt:new Date().toISOString(),reportName:'',reportData:''};let bs=get(K.bookings,[]);bs.unshift(booking);set(K.bookings,bs);let pays=get(K.payments,[]);pays.unshift({id:uid('PAY'),bookingId:booking.id,email:booking.email,amount:price,mode:booking.payment,status:booking.payment.includes('Online')?'Demo Paid':'Pending',date:new Date().toISOString()});set(K.payments,pays);addNotification('New booking received: '+booking.id);showMsg('bookingMsg','success',`Booking successful. Your Booking ID is ${booking.id}. Login with same email to track reports.`);f.reset();}
function registerPatient(e){e.preventDefault();const f=e.target;let ps=get(K.patients,[]);if(ps.find(p=>p.email===f.email.value.trim())){showMsg('authMsg','error','This email is already registered. Please login.');return}const p={id:uid('PT'),name:f.name.value.trim(),phone:f.phone.value.trim(),email:f.email.value.trim(),password:f.password.value,address:f.address?.value||''};ps.push(p);set(K.patients,ps);localStorage.setItem(K.session,JSON.stringify({type:'patient',email:p.email,name:p.name}));location.href='patient-dashboard.html'}
...
function uploadReport(id,input){const file=input.files[0];if(!file)return;const r=new FileReader();r.onload=()=>{let bs=get(K.bookings,[]);let b=bs.find(x=>x.id===id);if(b){b.reportName=file.name;b.reportData=r.result;b.status='Reported';set(K.bookings,bs);addNotification('Report uploaded for '+id);renderBookings();renderMetrics();alert('Report uploaded. Patient can download it now.')}};r.readAsDataURL(file)}
...
function savePatientEntry(e){
  ...
  const booking={
    ...
    patientName:f.patientName.value.trim(), age:f.age.value, gender:f.gender.value, phone:f.phone.value.trim(), email:f.email.value.trim(),
    ...
    rateType:f.rateType.value, createdAt:new Date().toISOString(), reportName:'', reportData:'', reportValues:null
  };
...
function buildReportHtml(b){
  const rows=(b.reportValues||[]).map(r=>{
    const main = `<tr><td><b>${escapeHtml(r.name)}</b></td><td>${escapeHtml(r.finding||'')}</td><td>${escapeHtml(r.unit||'')}</td><td>${escapeHtml(r.normal||'')}</td><td>${escapeHtml(r.comment||'')}</td></tr>`;
    const detail = (r.details||[]).map(d=>`<tr><td style="padding-left:26px">↳ ${escapeHtml(d.parameter||'')}</td><td>${escapeHtml(d.finding||'')}</td><td>${escapeHtml(d.unit||'')}</td><td>${escapeHtml(d.normal||'')}</td><td></td></tr>`).join('');
    return main + detail;
  }).join('');
  return `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Report ${b.billNo||b.id}</title><style>
  body{font-family:Arial,sans-serif;margin:0;color:#0b1736}.page{width:800px;margin:auto;padding:28px}.head{display:flex;align-items:center;border-bottom:3px solid #e30613;padding-bottom:12px}.head img{width:90px}.brand{margin-left:16px}.brand h1{margin:0;color:#062b59}.brand p{margin:4px 0;color:#e30613;font-weight:700}.info{display:grid;grid-template-columns:1fr 1fr;margin:18px 0;gap:8px}.info div{border:1px solid #ddd;padding:8px}table{width:100%;border-collapse:collapse;margin-top:12px}th{background:#062b59;color:white}td,th{border:1px solid #bbb;padding:10px;text-align:left}.foot{margin-top:40px;display:flex;justify-content:space-between}.note{font-size:12px;color:#555;margin-top:20px}.print{margin:20px auto;display:block;padding:10px 16px}@media print{.print{display:none}.page{width:auto}}</style></head><body><button class="print" onclick="window.print()">Print / Save PDF</button><div class="page">
  <div class="head"><img src="assets/khuntest-logo.png"><div class="brand"><h1>KHUNTEST LAB</h1><p>Accurate • Reliable • Trusted</p><div>Darbhanga, Bihar | +91 98765 43210</div></div></div>
  <h2>Diagnostic Test Report</h2><div class="info"><div><b>Bill No:</b> ${escapeHtml(b.billNo||b.id)}</div><div><b>Reporting Date:</b> ${escapeHtml(b.reportingDate||'')}</div><div><b>Patient Name:</b> ${escapeHtml(b.patientName||'')}</div><div><b>Age/Gender:</b> ${escapeHtml((b.age||'')+' / '+(b.gender||''))}</div><div><b>Contact:</b> ${escapeHtml(b.phone||'')}</div><div><b>Email:</b> ${escapeHtml(b.email||'')}</div><div><b>Ref. Doctor:</b> ${escapeHtml(b.doctor||'')}</div><div><b>Pathology:</b> ${escapeHtml(b.pathologyName||'BN-MAIN')}</div><div><b>Collection Date:</b> ${escapeHtml(b.collDate||b.date||'')}</div><div><b>Field Boy:</b> ${escapeHtml(b.staff||'')}</div></div>
  <table><thead><tr><th>Test Name</th><th>Finding</th><th>Unit</th><th>Normal Value</th><th>Comment</th></tr></thead><tbody>${rows}</tbody></table>
  <div class="info"><div><b>Gross Total:</b> ₹${Number(b.price||0).toFixed(2)}</div><div><b>Cash:</b> ₹${Number(b.cashReceived||0).toFixed(2)}</div><div><b>Card:</b> ₹${Number(b.cardReceived||0).toFixed(2)}</div><div><b>Discount:</b> ₹${Number(b.discount||0).toFixed(2)}</div><div><b>Balance/Dues:</b> ₹${Number(b.balanceDue||0).toFixed(2)}</div><div><b>Remarks:</b> ${escapeHtml(b.remarks||'')}</div></div>
  <div class="foot"><div>Checked By</div><div>Authorized Signatory</div></div><p class="note">This is a computer generated report. Please correlate clinically.</p></div></body></html>`;
}
function openReportPreview(){
  const b=updateReportValuesOnly(); if(!b){ alert('Please select booking.'); return; }
  const w=window.open('', '_blank'); w.document.write(buildReportHtml(b)); w.document.close();
}
function releaseGeneratedReport(){
  const b=updateReportValuesOnly(); if(!b){ alert('Please select booking.'); return; }
  const html=buildReportHtml(b);
  let bs=get(K.bookings,[]); let row=bs.find(x=>x.id===b.id);
  row.reportName='KhunTest_Report_'+(row.billNo||row.id)+'.html';
  row.reportData='data:text/html;charset=utf-8,'+encodeURIComponent(html);
  row.status='Reported';
  set(K.bookings,bs);
  addNotification('Report released for Bill '+(row.billNo||row.id)+' / '+row.patientName);
  renderMetrics(); renderBookings(); renderReportBookingSelect(); alert('Report released. Patient can download complete report with all values.');
}
