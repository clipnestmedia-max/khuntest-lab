...
function normalizeParameter(row, fallback = {}) {
  ...
  return {
    name,
    displayName: row.displayName || row.display_name || name,
    ...
    resultType: row.resultType || row.result_type || row.type || fallback.resultType || "",
    type: row.type || row.resultType || "",
    decimalPlaces: row.decimalPlaces ?? row.decimal_places ?? "",
    referenceRanges: Array.isArray(row.referenceRanges) ? row.referenceRanges : [],
    interpretationRules: Array.isArray(row.interpretationRules) ? row.interpretationRules : [],
    generalInterpretation: row.generalInterpretation || "",
    clinicalNotes: row.clinicalNotes || row.clinicalNote || "",
    recommendation: row.recommendation || "",
    reportComment: row.reportComment || "",
    autoFlagEnabled: row.autoFlagEnabled !== false,
    autoInterpretationEnabled: row.autoInterpretationEnabled !== false,
    showInterpretationOnReport: row.showInterpretationOnReport !== false,
    showClinicalNotesOnReport: row.showClinicalNotesOnReport !== false,
...
function normalizeTest(data) {
  ...
  return {
    ...
    department: data.department || category,
    category,
    price: Number(data.price ?? data.priceInr ?? data.price_inr ?? 0),
    sample,
    sampleType: data.sampleType || data.sample_type || sample,
    method: data.method || "",
    analyzer: data.analyzer || "",
    resultType: data.resultType || data.result_type || "text",
    decimalPlaces: data.decimalPlaces ?? data.decimal_places ?? "",
    defaultUnit: data.defaultUnit || data.default_unit || data.unit || "",
    allowedUnits: Array.isArray(data.allowedUnits) ? data.allowedUnits : [],
    referenceRanges: Array.isArray(data.referenceRanges) ? data.referenceRanges : [],
    interpretationRules: Array.isArray(data.interpretationRules) ? data.interpretationRules : [],
    generalInterpretation: data.generalInterpretation || "",
    clinicalNotes: data.clinicalNotes || data.clinicalNote || "",
    recommendation: data.recommendation || "",
    reportComment: data.reportComment || "",
    criticalValueEnabled: Boolean(data.criticalValueEnabled),
    autoFlagEnabled: data.autoFlagEnabled !== false,
    autoInterpretationEnabled: data.autoInterpretationEnabled !== false,
    showMethodOnReport: data.showMethodOnReport !== false,
    showSampleOnReport: data.showSampleOnReport !== false,
    showInterpretationOnReport: data.showInterpretationOnReport !== false,
    showClinicalNotesOnReport: data.showClinicalNotesOnReport !== false,
    displayOrder: Number(data.displayOrder || data.sortOrder || 0),
...
function normalizeResult(row) {
  return {
    ...
    selectedReferenceRange: row.selectedReferenceRange || null,
    flag: row.flag || "",
    severity: row.severity || "",
    generatedInterpretation: row.generatedInterpretation || "",
    finalInterpretation: row.finalInterpretation || "",
    interpretationEdited: Boolean(row.interpretationEdited),
    interpretationEditedBy: row.interpretationEditedBy || "",
    interpretationEditedAt: row.interpretationEditedAt || "",
    matchedInterpretationRuleId: row.matchedInterpretationRuleId || "",
    interpretationEngineVersion: row.interpretationEngineVersion || "",
    clinicalNote: row.clinicalNote || "",
    recommendation: row.recommendation || "",
    reportComment: row.reportComment || "",
    interpretationWarnings: Array.isArray(row.interpretationWarnings) ? row.interpretationWarnings : [],
    masterTestVersion: row.masterTestVersion || "",
    details: row.details || [],
    isHeading: Boolean(row.isHeading || row.heading || row.type === "heading")
  };
}
